...
import re,time,math,calendar,cgi,atom,smtplib,ldap
from datetime import datetime,timedelta
import gdata.calendar.service
import gdata.calendar
officers = {'President':'vfounta', \
            'Couples Resource Coordinator':'gordonov', \
            'Parents Resource Coordinator':'jpwhit05', \
            'Community & Sustainability Coordinator':'csauer', \
            'Social Chair':'gysu', \
            'Secretary-Treasurer':'hannahac', \
            'Partners Community Coordinator':'bichoy', \
            'Information & Technology Coordinator':'achavda', \
            'Residental Life Associate':'naomic'}

superusers = {'Westgate President':'vfounta', \
            'Information & Technology Coordinator':'achavda', \
            'Residental Life Associate':'naomic'}
...
admin_name = 'House Director'
admin_email = 'naomic@mit.edu'
admin_apt = '1502'
...
def Goog2Dt(s):
    f = '%Y-%m-%dT%H:%M:%S.000'
    return datetime(*(time.strptime(s[:-6],f)[:6]))
...
def ParseTime(s):
    ...
    if h == 12 and t == 'a' or h == 24:
        h = 0
    return '%02d:%02d:00.000%s' % (h,m,Tz2Str())
...
#----------------------------------------------------------------------------------
def AddDay(s):
    return Dt2Goog(Goog2Dt(s)+timedelta(1))
...
#----------------------------------------------------------------------------------
def SortEvent(e1,e2):
    start1 = e1.when[0].start_time
    start2 = e2.when[0].start_time
...
def InsertEvent(cs,url,title,content,where,start,end,items):
    ...
    for key,val in items.items():
        event.extended_property.append(gdata.calendar.ExtendedProperty(name=str(key),value=str(val)))
    newevent = cs.InsertEvent(event,url)
    return newevent
...
def SetItems(event,items):
    for key,val in items.items():
        n = len(event.extended_property)
        flag = False
        for i,prop in zip(range(n),event.extended_property):
            if prop.name == key:
                event.extended_property[i].value = val
                flag = True
                break
        if not flag:
            event.extended_property.append( \
                gdata.calendar.ExtendedProperty(name=key,value=val))
    return event
...
def ProcessForm(form,ispay=False):
    ...
    if bool(date) and bool(pstart) and bool(pend):
        ...
        if area != '' and IsConflict(cs,url,start,end):
            errors.append('There is another event already scheduled during your time range at your location. Note there is known bug with daylight savings time. If the calendar shows a back to back event but your slot is free, try moving the event by an hour. This can fix the problem if you are scheduling an event that takes place after a switch in daylight savings time.')
